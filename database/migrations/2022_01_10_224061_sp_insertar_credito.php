...
class SpInsertarCredito extends Migration
{
    ...
    public function up()
    {
        ...
        in tipo_p text, in estado_p int, in sucursal_p text, in res_sunat_p text,in cod_sunat_p text, in fecha_p datetime)
        begin
            if(estado_p is null) then
              insert into credito_debito_sunat(venta_id, serie, correlativo, tipo, sucursal, created_at, updated_at) 
             values(venta_id_p,serie_p, correlativo_p, tipo_p, sucursal_p, fecha_p, fecha_p);
              update ventas set anulado_cod = correlativo_p where id = venta_id_p;
             else 
              update credito_debito_sunat set estado = estado_p , 
              res_sunat = res_sunat_p, cod_sunat = cod_sunat_p, updated_at = fecha_p
              where serie = serie_p;
            end if;
        END";
